...
export class PerformanceStats {
    ...
    updateFromStatsGL(now) {
        try {
            ...
            // Debug log - reduced frequency now that we have the right data
            if (Math.random() < 0.02) { // 2% chance to log for debugging
                console.log('stats-gl debug (FIXED):', {
                    lastValue: this.statsGL.lastValue,
                    extractedFPS: this.currentFPS,
                    extractedGPU: this.currentGPU,
                    extractedCPU: this.currentCPU
                });
            }
